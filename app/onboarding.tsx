...
export default function Onboarding() {
  ...
  const canNext = step === 0 ? true : step === 1 ? !!country : interests.size > 0;
  ...
  const finish = (skip?: boolean) => {
    ...
    completeOnboarding({ country: skip ? null : emoji, interests: nextInterests });
    if (configured && user) {
      supabase
        .from('profiles')
        .update({ country: skip ? null : emoji, interests: nextInterests })
        .eq('id', user.id)
        .then(({ error }) => {
  ...
  return (
    <View style={{ flex: 1, backgroundColor: c.paper, paddingTop: insets.top + 12 }}>
      {/* progress */}
      ...
      <View style={{ flex: 1, paddingHorizontal: 24 }}>
        {step === 0 && <Welcome />}
        {step === 1 && <PickRegion region={country} setRegion={setCountry} />}
        {step === 2 && <PickInterests interests={interests} toggle={toggle} />}
      </View>
